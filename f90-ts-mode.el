...
(defcustom f90-ts-indent-fun 3
  "Extra indentation applied to function and subroutine bodies."
  :type  'integer
  :safe  'integerp
  :group 'f90-ts-indent)

(defcustom f90-ts-indent-if 3
  "Extra indentation applied to IF and SELECT CASE."
  :type  'integer
  :safe  'integerp
  :group 'f90-ts-indent)

(defcustom f90-ts-indent-do 3
  "Extra indentation applied to DO, BLOCK, ASSOCIATE blocks."
  :type  'integer
  :safe  'integerp
  :group 'f90-ts-indent)
    ...
    (treesit-font-lock-rules
     ...
     '(;; capture comments starting with !$, which are openmp statements
       ((comment) @f90-ts-font-lock-openmp-face
        (:pred f90-ts--looking-at-openmp-p @f90-ts-font-lock-openmp-face))
       )))
...
(setq f90-ts-font-lock-rules
      ;; "Combined font-lock rules.
      (list
...
(defun f90-ts--openmp-comment-is ()
  ...
  (lambda (node parent bol &rest _)
    (and (f90-ts--node-type-p node "comment")
         (f90-ts--looking-at-openmp-p node))))


(defun f90-ts--looking-at-openmp-p (node)
  "Check if start of NODE is on a line starting with optional whitespace and !$,
which is an openmp statement."
  (save-excursion
    (goto-char (treesit-node-start node))
    (beginning-of-line)
    ;;(f90-ts-log :indent "at: %d, %d" (point) (line-end-position))
    ;;(f90-ts-log :indent "looking at: %s" (buffer-substring (point) (line-end-position)))
    (looking-at "\\s-*!\\$")))
...
(defun f90-ts--align-continued-children (parent)
  "Depending on context, we might need to drop children of PARENT
or use grandparent.
...
(defun f90-ts--indent-toplevel-offset (node parent _bol)
  ...
  (let ((grandparent (treesit-node-parent parent)))
    ...
    (if (member (treesit-node-type grandparent)
...
(defun f90-ts--node-type-p (node type)
  "If type is nil, return true and ignore node. Otherwise return true if node is non-nil
and is of type TYPE."
  (if type
...
(defun f90-ts--first-node-on-line (node)
  "Return the first node on the line at which NODE is placed."
  (save-excursion
    (goto-char (treesit-node-start node))
    (back-to-indentation)
...
statement is found."
  (let ((head (f90-ts--first-node-on-line node)))
    (while (string= (treesit-node-type head) "&")
      ...
      (let ((prev (treesit-node-prev-sibling head)))
        ...
        (setq head (f90-ts--first-node-on-line prev))))
    head))
  ...
  (lambda (node parent bol &rest _)
    ...
    (when (or (string= msg "first") (string= msg "final catch"))
      (let* ((prev-stmt (f90-ts--previous-stmt node parent))
        ...
        (f90-ts-inspect-node :indent node "fail-info[node]")
        (f90-ts-inspect-node :indent parent "fail-info[parent]")
        (f90-ts-inspect-node :indent prev-stmt "fail-info[prevstmt]")
        (f90-ts-inspect-node :indent child0 "fail-info[child0]")
        (f90-ts-inspect-node :indent sibling0 "fail-info[firstsib]")
        (f90-ts-inspect-node :indent npsib "fail-info[npsib]")
        (f90-ts-inspect-node :indent ppsib "fail-info[ppsib]")
        ))
    nil))
...
(setq f90-ts-indent-rules-test-first
      `(;; for testing purposes
...
(setq f90-ts-indent-rules-comments
...
(setq f90-ts-indent-rules-argument
  `(;; we compute absolute column position, using parents column as anchor is not useful
    ,@(f90-ts-indent-rules-info "argument")
...
(setq f90-ts-indent-rules-prog-mod
...
(setq f90-ts-indent-rules-function
      `(;; functions and subroutine bodies
        ...
        ((parent-is "subroutine") parent f90-ts-indent-fun)
        ((parent-is "function") parent f90-ts-indent-fun)
        ((n-p-ps nil nil "subroutine") parent f90-ts-indent-fun)
        ((n-p-ps nil nil "function") parent f90-ts-indent-fun)
        ))


(setq f90-ts-indent-rules-interface
      `(;; (abstract) interface bodies
        ,@(f90-ts-indent-rules-info "interface")
        ((node-is "end_interface_statement") parent 0)
        ((parent-is "interface") parent f90-ts-indent-fun)
        ((n-p-ps nil "ERROR" "interface") parent f90-ts-indent-fun)
        ))


(setq f90-ts-indent-rules-derived-type
      `(;; type definitions
        ...
        ((parent-is "derived_type_procedures") parent f90-ts-indent-fun)
        ((parent-is "derived_type_definition") parent f90-ts-indent-fun)
        ))


(setq f90-ts-indent-rules-if
      `(;; if-then-else statements
        ...
        ((n-p-ps nil                "if_statement"  "if")     parent f90-ts-indent-if)
        ((n-p-ps nil                "if_statement"  "else")   parent f90-ts-indent-if) ; line after else, with empty else block
        ((n-p-ps nil                "else_clause"   "else")   parent f90-ts-indent-if) ; line after else, with non-empty else block
        ((n-p-ps nil                "elseif_clause" "elseif") parent f90-ts-indent-if)

        ((n-p-ps "elseif_clause"    "ERROR" "if") previous-stmt-anchor 0)
        ((n-p-gp "elseif_clause"    "ERROR" nil)  previous-stmt-anchor (f90-ts--minus-offset ,f90-ts-indent-if))

        ((n-p-ps "else_clause"      "ERROR" "if")     previous-stmt-anchor 0)
        ((n-p-ps "else_clause"      "ERROR" "elseif") previous-stmt-anchor 0)
        ((n-p-gp "else_clause"      "ERROR" nil)      previous-stmt-anchor (f90-ts--minus-offset ,f90-ts-indent-if))
        ((n-p-gp "else"             "ERROR" nil)      previous-stmt-anchor (f90-ts--minus-offset ,f90-ts-indent-if))

        ((n-p-ps nil                "ERROR" "if")     previous-stmt-anchor f90-ts-indent-if)
        ((n-p-ps nil                "ERROR" "elseif") previous-stmt-anchor f90-ts-indent-if)
        ((n-p-ps nil                "ERROR" "else")   previous-stmt-anchor f90-ts-indent-if)
        ))


(setq f90-ts-indent-rules-control
      `(;; control statements
        ,@(f90-ts-indent-rules-info "control")
        ((n-p-gp    "end_do_loop" "do_loop" nil)  parent 0)
        ((n-p-ps    nil           "do_loop"           "do") parent f90-ts-indent-do)  ;; proper do block (with or without while)
        ((n-p-ps    nil           "ERROR"             "do") previous-stmt-anchor f90-ts-indent-do)
        ;;((n-p-ns-ps nil         "ERROR"             nil   "do") previous-stmt-anchor f90-ts-indent-do)

        ((n-p-gp "end_block_construct_statement" "block_construct" nil)     parent 0)
        ((n-p-ps nil                             "block_construct" "block") parent f90-ts-indent-do)
        ((n-p-ps nil                             "ERROR"           "block") previous-stmt-anchor f90-ts-indent-do)
        ...
        ((n-p-ps nil                       "association"         "associate") parent f90-ts-indent-do)
        ((n-p-ps nil                       "associate_statement" "associate") parent f90-ts-indent-do)
        ((n-p-ps nil                       "ERROR"               "associate") column-0 f90-ts--align-continued-assoc-error)
        ))


(setq f90-ts-indent-rules-catch-all
...
;; hack for development, this updates the indent rules, after reevaluating the buffer
;; and toggle the f90-ts mode twice in the source file
;; later on: replace setq by defvar for all indent rules, but then restart emacs becomes
;; necessary for changes to take effect
(setq f90-ts-indent-rules
  `((fortran
     ,@f90-ts-indent-rules-test-first
     ,@f90-ts-indent-rules-openmp
     ,@f90-ts-indent-rules-comments
     ,@f90-ts-indent-rules-argument
...
(define-derived-mode f90-ts-mode prog-mode "F90[TS]"
  "Major mode for editing Fortran 90+ files using tree-sitter.
This mode uses tree-sitter for syntax highlighting, indentation and
other functions."
  :group 'f90-ts
  (interactive)
...
Each entry enables a class of info or debug messages in `f90-ts-log'."
  :type '(set (const :tag "info" :info)
              (const :tag "debug" :debug)
              (const :tag "indent" :indent)
              (const :tag "fontlock" :fontlock)
              (const :tag "complete" :complete)
              (const :tag "auxiliary" :auxiliary))
...
FMT + ARGS are passed to `format' and like message arguments."
  (when (memq category f90-ts-log-categories)
...
;; automatically reload f90-ts-mode when saving this file
(defun f90-ts--reload-on-save ()
  "Reload f90-ts-mode.el and refresh indent rules in all f90-ts-mode buffers."
  (when (string-equal
         (file-truename buffer-file-name)
         (file-truename (find-lisp-object-file-name 'f90-ts-mode nil)))
    ;; Re-evaluate the elisp file (redefines functions + variables)
    (eval-buffer)

    ;; Update existing f90-ts-mode buffers
    (dolist (buf (buffer-list))
      (with-current-buffer buf
        (when (derived-mode-p 'f90-ts-mode)
          (progn
            (message "this buf %s" buf)
            ;; reapply indent rules
            (setq-local treesit-simple-indent-rules f90-ts-indent-rules)))))

    (message "reloaded f90-ts-mode.el via eval-buffer and updated all f90-ts-mode buffers")))

(add-hook 'after-save-hook #'f90-ts--reload-on-save)
